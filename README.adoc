...
=== Splitting long slides without impacting other backends
To mark slides split points you can use preprocessor conditionals combinded with backend declaration.+
Breaking points are set with slideless titles `=== !` wrapped in a conditionnal : `ifdef::backend-revealjs[=== !]`. 
In the end, the whole document has to be compiled with the backend option : `-b revealjs`
...
----
Compiled with `asciidoctor -T asciidoctor-reveal.js/templates/slim/ -b revealjs mydoc.adoc`
