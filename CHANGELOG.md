...
### Bug Fixes
  ...
  * Fix 3 JS converter parity gaps surfaced by the asciidoc-testkit corpus: the `icon:` macro rendered as literal `[name]` text instead of `<img>` whenever `:icons:` was set with no value (`''` is falsy in JS but truthy in Ruby, so a plain truthiness check on the attribute's value took the wrong branch); a bibliography anchor with no custom reftext (`[[[id]]]`) rendered its bracketed placeholder as `[null]` instead of `[]` (JS template literals stringify `null`, Ruby's `nil.to_s` doesn't); and a `footnoteref:` reused outside any `<section>` (embedded conversion, or content before the first slide) always got renumbered to 1 instead of reusing its original number, because the per-slide dedupe cache was never written in that case
...
### Build
  ...
  * Replace the `asciidoctor-doctest` gem with [asciidoc-testkit](https://github.com/ggrossetie/asciidoc-testkit) (`asciidoc-testkit-cli` npm devDependency) as the fixture-testing tool: `rake testkit:test`/`testkit:update` run the bundled 37 generic AsciiDoc-construct families (`test/expected-testkit/<family>/<case>.html`, embedded conversion) plus the project's own `standalone` family (`test/expected/standalone/<case>.html`, standalone conversion, narrowed to the relevant fragment via each case's `<name>.config.json` `select` sidecar instead of the old xpath `:include:` comment). `test/fixtures/` holds the input side (matching asciidoc-testkit's own `--fixtures` flag); `test/expected(-testkit)/` holds the expected output, deliberately not also named "fixtures". Dropping `asciidoctor-doctest` also drops its `nokogiri` dependency, which had pinned Windows CI to Ruby 3.2
  * Reorganize `examples/`: split it into `release-notes/` (the release showcase demos, still published to the Netlify demos site) and `test/fixtures/standalone/` (everything else) — the latter doubles as a browsable gallery of real presentations and as the asciidoc-testkit `standalone` family (cases that only exist to pin down a specific behavior — docinfo, slide numbers, syntax highlighters, ... — live there too now, there's no separate family for them anymore)
  ...
  * Add `rake testkit:test:js` to run the same asciidoc-testkit corpus against the JS converter (`js/bin/asciidoctor-revealjs`) for a Ruby/JS parity check that also covers the 37 generic AsciiDoc-construct families, unlike `js/test/examples.test.js`, which only covers `test/fixtures/standalone/` and `release-notes/`
  * Wire `testkit:test:js` into CI, on the primary Linux/CRuby leg only (it exercises OS-independent Node.js code, so one platform is enough) and `continue-on-error: true` — 2 permanent, documented gaps remain (`source-prettify`/`source-rouge`, third-party Ruby-only syntax highlighters never ported to the JS converter), not something a future commit can regress-fix
  * Fix 13 false-positive `standalone` family failures in `rake testkit:test:js`: `revealjsdir`'s default intentionally differs between the two converters (`reveal.js/` outside Node, `node_modules/reveal.js/` in a Node.js environment), but neither the Ruby nor the JS `testkit.rake` converter invocation overrode it, so every case's `<script src>`/`<link href>` differed for that reason alone, independent of any real parity gap. `JS_CONVERTER` now pins `-a revealjsdir=reveal.js@` (the `@` keeps it a soft default a document's own `:revealjsdir:` can still override, needed by `standalone/mathjax-cdn`) to match what `RUBY_CONVERTER` — and the expected fixtures it generated — already default to
