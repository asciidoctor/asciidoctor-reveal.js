...
### Build
  ...
  * Replace the `asciidoctor-doctest` gem with [asciidoc-testkit](https://github.com/ggrossetie/asciidoc-testkit) (`asciidoc-testkit-cli` npm devDependency) as the fixture-testing tool: `rake testkit:test`/`testkit:update` run the bundled 37 generic AsciiDoc-construct families (`test/expected/<family>/<case>.html`, embedded conversion) plus two reveal.js-specific families — `revealjs` and `revealjs-examples` (`test/expected-revealjs/<family>/<case>.html`, standalone conversion, narrowed to the relevant fragment via each case's `<name>.config.json` `select` sidecar instead of the old xpath `:include:` comment). `test/fixtures-extra/` holds the input side (matching asciidoc-testkit's own `--fixtures` flag); `test/expected(-revealjs)/` holds the expected output, deliberately not also named "fixtures". Dropping `asciidoctor-doctest` also drops its `nokogiri` dependency, which had pinned Windows CI to Ruby 3.2
