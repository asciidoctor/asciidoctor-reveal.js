...
[IMPORTANT]
====
While you can use the `fragment` role on any element,
we recommend using the `step` option to declare a fragment (see below). +
It ensures better portability and avoids CSS conflicts when using another converter.
====
...
Since it's not possible to define attributes or options on text and inline elements, you can use the `step` role.
...
And here's another example where we apply the `step` option to different elements:
