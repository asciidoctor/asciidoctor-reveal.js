...
[NOTE]
Alternatively, you can use {url-rouge}[Rouge], {url-coderay}[Coderay], or {url-pygment}[Pygments] as syntax highlighters if you are using the Asciidoctor / Ruby / Bundler toolchain (not Asciidoctor.js / JavaScript / npm).
Check the {url-project-examples}[project's example corpus] for examples and notes about what needs to be done for them to work.
