...
Read {url-revealjs-doc}/backgrounds/[the reveal.js documentation] to understand what attributes can be set.
Keep in mind that for a title slide, you must replace `data-` with `title-slide-` in the name of the attribute.

See xref:attachment$title-slide-image.adoc[title-slide-image.adoc] for an example of using these attributes.

The title slide is also given a special `title` CSS class to help with template customization.
