...
Additional examples can be found in the AsciiDoc files (.adoc) in the {url-project-examples}[project's example corpus].
...
In the previous snippet, we create a slide titled Slide One with bullets and another titled Slide Two with centered text (reveal.js`' default behavior) and link:{url-revealjs-doc}/speaker-view/[speaker notes].
Other syntax exists to create speaker notes; see xref:attachment$speaker-notes.adoc[speaker-notes.adoc].
...
Speaker notes support configurable layouts and are opened by pressing the `s` key:
...
See xref:attachment$concealed-slide-titles.adoc[concealed-slide-titles.adoc].
...
To mark slide split points, you can use preprocessor conditionals combined with a backend declaration.
Breaking points are set using titleless slides (`=== !`) wrapped in a conditional: `ifdef::backend-revealjs[=== !]`.
The whole document then has to be compiled with the backend option `-b revealjs`.
