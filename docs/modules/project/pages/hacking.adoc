...
* the bundled 37 generic AsciiDoc-construct families (`test/expected-testkit/<family>/<case>.html`), converted embedded;
* the project's own `standalone` family (`test/expected/standalone/<case>.html`), converted standalone and narrowed to the relevant fragment via each case's `<name>.config.json` `select` sidecar.

`test/fixtures/` holds the input side (matching asciidoc-testkit's own `--fixtures` flag); `test/expected(-testkit)/` holds the expected output — deliberately not also named "fixtures", to keep the two apart at a glance.

`rake testkit:update` regenerates the expected files from the current converter's output — review the diff before committing.

=== Custom tests

Presentations in `test/fixtures/standalone/` double as a browsable gallery of real presentations and as regression tests. A file with no matching expected file under `test/expected/standalone/` is simply not (yet) covered by a regression test — that's fine, it's still a valid example.
