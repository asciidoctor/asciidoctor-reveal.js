...
* the bundled 37 generic AsciiDoc-construct families (`test/fixtures/<family>/<case>.html`), converted embedded;
* the reveal.js-specific `revealjs` and `revealjs-examples` families (`test/fixtures-revealjs/<family>/<case>.html`), converted standalone and narrowed to the relevant fragment via each case's `<name>.config.json` `select` sidecar.

`rake testkit:update` regenerates the expected files from the current converter's output — review the diff before committing.

=== Custom tests

Presentations in the `examples/` directory that are also regression tests (not pure showcase demos) have a matching expected file under `test/fixtures-revealjs/revealjs-examples/`.
