...
Presentations in the `examples/` directory that are also regression tests (not pure showcase demos) have a matching expected file under `test/expected-revealjs/revealjs-examples/`.
Presentations that exist only to pin down a specific behavior, with no independent showcase value, live directly under `test/fixtures-extra/revealjs/` instead, so `examples/` stays a gallery of real presentations.
